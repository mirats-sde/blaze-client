...
const AnalyticsContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    allSessions?.forEach((session) => {
      ...
      let creationDate = sd?.date.toDate().toLocaleDateString("en-us", options);
          ...
          if (String(que?.question_id) === String(res?.question_id)) {
            ...
            if (que?.question_type === "Numeric - Open-end") {
              ...
              survey?.qualifications?.questions?.map((que) => {
                if (String(que?.question_id) === String(res?.question_id)) {
                  que?.conditions?.valid_responses?.map((resp) => {
                    ageRange.push(`${resp?.from}-${resp?.to}`);
                  });
                }
              });
              ...
              key =
                que?.lang?.[survey?.country?.code].options[res?.user_response];
            }
